...
<div x-data="{ showOfferingSlip: false }">
    <div class="flex justify-between items-center mb-4">
        ...
        <button @click="showOfferingSlip = !showOfferingSlip" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">
        ...
        <div x-data="donationForm()" x-init="init()">
            ...
            <form @submit.prevent="submitNormal" class="mb-10 bg-gray-50 p-4 rounded shadow">
                ...
                <div class="mb-4">
                    ...
                    <select x-model="donation.name" required class="mt-1 block w-full p-2 border border-gray-300 rounded">
                ...
                <div class="grid grid-cols-3 gap-4 mb-4">
                    <template x-for="offering in offerings.filter(o => o.parent_id === null)" :key="offering.id">
                        <div>
                            <label class="block text-md md:text-base lg:text-md font-medium text-gray-700" x-text="offering.category.toUpperCase()"></label>
                            <div class="relative mt-1">
                                <!-- Peso sign prefix -->
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-700">₱</span>
                                <!-- Input field -->
                                <input type="number" step="any"
                                       x-model="offering.amount"
                ...
                <div class="flex flex-col sm:flex-row gap-3 justify-end mt-6">
                    ...
                    <button type="submit" class="submit-btn px-4 py-2 bg-blue-600 text-white rounded text-md md:text-base lg:text-lg transition-all duration-300 cursor-pointer flex-1 sm:flex-none">
                        Save Offering
                    </button>
...
<div id="offeringUploadModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-60">
    <div class="bg-white p-6 rounded-2xl shadow-xl w-full max-w-lg relative z-70">
        ...
        <button type="button"
            onclick="document.getElementById('offeringUploadModal').classList.add('hidden')"
                ...
                @foreach($currentFund as $fund)
                    @if($fund->total > 0) <!-- Only include rows where the total is greater than zero -->
                        <tr class="border-b hover:bg-gray-100">
                            ...
                            <td class="border px-6 py-4 text-right">₱{{ number_format($fund->total, 2) }}</td>
                        </tr>
                    @endif
                @endforeach
...
<div x-data="donationForm()" x-init="init()">
    ...
    <div class="mb-6">
        ...
        <form method="GET" action="{{ url()->current() }}" class="flex justify-start">
            ...
            <select name="category" id="category" class="border border-gray-300 rounded pr-8 py-2 ml-3" onchange="this.form.submit()">
                ...
                <tbody class="text-center">
                    @foreach($offeringsList as $offering)
                        <tr class="border">
                            ...
                            <td class="border px-4 py-2">{{ $offering->member_name !== "Visitor Visitor" ? $offering->member_name : "Donator" }}</td>
                            <td class="border px-4 py-2">{{ $offering->offer_category }}</td>
                            <td class="border px-4 py-2 text-right">₱{{ number_format($offering->amount, 2) }}</td>
