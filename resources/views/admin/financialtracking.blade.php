    ...
    <div class="bg-white shadow-lg p-6 rounded-lg" x-data="financialTrackingData()" x-init="init()">
        ...
        <div class="border-b border-gray-300">
            <div class="flex space-x-2 overflow-x-auto scrollbar-hide">
                <template x-for="tab in ['offerings', 'expenses', 'confirmation', 'overview', 'setup', 'summary', 'report']">
                            ...
                            <template x-for="(row, index) in partitions" :key="index">
                                <tr>
                                    ...
                                    <td class="border px-4 py-2">
                                        <select class="w-full p-1 border rounded" x-model="row.category" @change="saveCategorySelection(row)">
                                    ...
                                    <td class="border px-4 py-2">
                                        <input type="number" class="w-full p-1 border rounded text-center" x-model="row.partition" @input="calculatePartition(row)">
                                    </td>
                                    <td class="border px-4 py-2">
                                        <select class="w-full p-1 border rounded" x-model="row.selectedOfferings" @change="saveOfferingSelection(row)" multiple>
                                    ...
                                    <td class="border px-4 py-2">
                                        <textarea class="w-full p-1 border rounded" x-model="row.description" readonly rows="2"></textarea>
                                    </td>
                                    <td class="border px-4 py-2 text-center">
                                    <button @click="deletePartitionRow(row.id, index)" 
                                    ...
                                    .delete-btn:hover {
                                        ...
                                        background-color: #ef4444;    /* Tailwind red-500 hover */
                                    }
                                    </style>
                                    </td>
                                </tr>
                        ...
                        <tbody>
                            <template x-for="(row, index) in offerings" :key="index">
                                <tr>
                                    <td class="border px-4 py-2 text-center" x-text="index + 1"></td>
                                    <td class="border px-4 py-2">
                                        <input type="text" class="w-full p-1 border rounded" x-model="row.category" placeholder="Category Name">
                                    </td>
                                    <td class="border px-4 py-2">
                                        ...
                                        <select class="w-full p-1 border rounded mt-1"
                                            ...
                                            <template x-for="parent in offerings.filter(o => o.id !== row.id && !o.is_subcategory)" :key="parent.id">
                                                <option :value="parent.id" x-text="parent.category"></option>
                                            </template>
                                        </select>
                                    </td>
                                    <td class="border px-4 py-2 text-center">
                                    <button @click="deleteOfferingRow(row.id, index)" 
                        ...
                        <tbody>
                            <template x-for="(row, index) in expenses" :key="index">
                                <tr>
                                    ...
                                    <td class="border px-4 py-2 text-center">
                                    <button @click="deleteExpenseRow(row.id, index)" 
...
<script>
    ...
    return {
        ...
        addPartitionRow(savedRow = null) {
            this.partitions.push({
                id: savedRow?.id || null,
                category: savedRow?.category || (this.expenses?.[0]?.description || ''),  // ⬅️ Only default if empty
                partition: savedRow?.partition || 0,
                selectedOfferings: savedRow?.selectedOfferings || [],
                description: savedRow?.description || ''
            });
        },
        saveCategorySelection(row) {
        ...
        addOfferingRow() {
            this.offerings.push({
                ...
                parent_id: null,
                branch_id: this.adminBranchId // Use the admin's branch ID directly
            });
        },
    ...
    if (data.success) {
        ...
        Swal.fire('Error', data.error || 'Failed to delete.', 'error');
            ...
            saveCategorySelection(row) {
                localStorage.setItem('selectedCategory_' + row.id, row.category);
                console.log('Saved category for row ID', row.id, ':', row.category);
            },
...
loadSelections() {
    this.partitions.forEach(row => {
        const savedCategory = localStorage.getItem('selectedCategory_' + row.id);
        if (savedCategory) {
            row.category = savedCategory;
            console.log('Loaded category for row ID', row.id, ':', row.category);
        }
...
saveOfferingChanges() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch('/financial-tracking/offerings', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
